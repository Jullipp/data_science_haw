...
# Ist es sinvoll alle Bewertungen in die Analyse einfließen zu lassen?
```{r}
all2 <- all %>%
...
```{r}
user_w_author <- split(all2$`Book-Author`,all2$`User-ID`) 
```
...
```{r}
trans.rules <- apriori(trans, parameter=list(supp=0.01, conf=0.3, target="rules"))
```

# Assoziationsanalyse mit Autoren-Namen
```{r}
head(inspect(subset(trans.rules)), 50)
```
...
```
```{r}
user_w_author2 <- split(all3$`Book-Author`,all3$`User-ID`) 
```
