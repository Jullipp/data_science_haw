...
books1 <- books %>%
  ...
  mutate(`Book-Author` = str_to_lower(`Book-Author`), 
         ...
         `Book-Author` = str_remove(`Book-Author`, "[a-z]\\."),
...
```{r}
books2 <- books1 %>%
  mutate(`Book-Author` = str_remove(`Book-Author`, "[a-z]\\."))
```
...
Update: Ich habe mir den nötigen Code zusammengebastellt mutate(`Book-Author` = str_remove(`Book-Author`, "[a-z]\\.")). Tatsächlich hat mir nur das Wissen gefehlt, wie ich dem System klarmache, dass ich mit "." auch wirklich einen Punkt meine und es nicht ein Stammplatzhalter für alle möglichen Zeichen sein soll. Da jedes Mal immer nur der erste Buchstabe entfernt wurde, der auf einem "." endet, habe ich den Datensatz dreimal mit diesem Befehl bereinigt und gehe jetzt davon aus, dass die meisten der Kürzel bereinigt wurden. Es ist keine sehr elegante Methode drei mal den Befehl zu wiederholen, aber es erfüllt meine Wünsche. Wenn es doch nur dafür sorgen könnte, dass ich nicht die ganze Zeit bis in den Morgen an Data Science rumsitze und mir den Kopf zerbreche `seufz`.

Nun habe ich nur noch das Problem, dass bei manchen Autor*innen die Zweitnamen ausgeschrieben wurden. Dies wird in der folgenden Warenkorbanalyse bei Patricia Daniels Cornwell sichtbar. Alle Namen bis auf den ersten Buchstaben zu kürzen wäre zu radikal und ungenau. Dann könnte RStudio nicht mehr zwischen Tom Alby und Thomas Anders unterscheiden. Nur den Zweitnamen entfernen geht auch schlecht, da ich damit auch Gefahr laufen würde die Nachnamen von Autor'innen ohne Zweitnamen zu entfernen. Das ist wohl der Punkt, an dem ich mich mit meinem Datensatz zufrieden stellen muss.
